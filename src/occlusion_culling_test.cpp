...
int main(int argc, char **argv)
{
    ...
    ros::Publisher originalCloudPub =
        nh.advertise<sensor_msgs::PointCloud2>("original_pointcloud", 100);
    ros::Publisher occludedCloudPub =
        nh.advertise<sensor_msgs::PointCloud2>("occluded_pointcloud", 100);
    ros::Publisher frustumCloudPub =
        nh.advertise<sensor_msgs::PointCloud2>("frustum_pointcloud", 100);
    ...
    ros::Publisher generagedPathPub =
        nh.advertise<visualization_msgs::Marker>("generated_path", 10);
    ...
    while (!feof(file))
    {
        ...
        geometry_msgs::Pose sensorViewPointPose =
            uav2camTransformation(robotLocation.pose, sensorRPY2Robot, sensorXYZ2Robot);
    ...
    pcl::toROSMsg(*occludedCloudPtr,
                  cloud2);  //cloud of the not occluded voxels (blue) using occlusion culling
    pcl::toROSMsg(*frustumCloudPtr,
                  cloud5);  //cloud of the not occluded voxels (blue) using occlusion culling
