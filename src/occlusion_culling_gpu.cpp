...
#include "culling/occlusion_culling_gpu.h"
...
OcclusionCullingGPU::OcclusionCullingGPU(ros::NodeHandle& n,
                                         pcl::PointCloud<PointInT>::Ptr& cloudPtr)
    : nh(n), fc(true)
{
    fov_pub = n.advertise<visualization_msgs::MarkerArray>("fov", 10);
    cloud = pcl::PointCloud<PointInT>::Ptr(new pcl::PointCloud<PointInT>);
...
OcclusionCullingGPU::OcclusionCullingGPU(std::string modelName) : model(modelName), fc(true)
{
    cloud = pcl::PointCloud<PointInT>::Ptr(new pcl::PointCloud<PointInT>);
    filtered_cloud = pcl::PointCloud<PointInT>::Ptr(new pcl::PointCloud<PointInT>);
    cloudCopy = pcl::PointCloud<PointInT>::Ptr(new pcl::PointCloud<PointInT>);
...
OcclusionCullingGPU::OcclusionCullingGPU() : model(NULL), fc(true)
{
    cloud = pcl::PointCloud<PointInT>::Ptr(new pcl::PointCloud<PointInT>);
    filtered_cloud = pcl::PointCloud<PointInT>::Ptr(new pcl::PointCloud<PointInT>);
    cloudCopy = pcl::PointCloud<PointInT>::Ptr(new pcl::PointCloud<PointInT>);
        ...
        for (int jj = min_b.y(); jj <= max_b.y(); ++jj)
        {
            for (int ii = min_b.x(); ii <= max_b.x(); ++ii)
            {
                ...
                if (index1 != -1)
                {
                    ...
                    Eigen::Vector3i ijk_in_Original = voxelFilterOriginal.getGridCoordinates(
                                            ...
                                            geometry_msgs::Pose cameraPose)
{
    ...
    {
        val =
            transformedPointCloud.at(j)
                .x;  //depth (it is at x axis because of the frustum culling camera pose requirement)
        pointError = 0.0000285 * val * val;
        errorSum += pointError;
    }
...
void OcclusionCullingGPU::SSMaxMinAccuracy(std::vector<geometry_msgs::PoseArray> sensorsPoses)
{
    ...
    for (int i = 0; i < sensorsPoses.size(); i++)
    {
        ...
        {
            tf::Quaternion qt(
                sensorsPoses[i].poses[j].orientation.x, sensorsPoses[i].poses[j].orientation.y,
                sensorsPoses[i].poses[j].orientation.z, sensorsPoses[i].poses[j].orientation.w);
            double r, p, y;
    ...
    maxAccuracyError =
        0.0000285 * max * max;  // the standard deviation equation is taken from paper
...
void OcclusionCullingGPU::transformPointMatVec(tf::Vector3 translation, tf::Matrix3x3 rotation,
                                               geometry_msgs::Point32 in,
                                               geometry_msgs::Point32& out)
{
    double x =
        rotation[0].x() * in.x + rotation[0].y() * in.y + rotation[0].z() * in.z + translation.x();
    double y =
        rotation[1].x() * in.x + rotation[1].y() * in.y + rotation[1].z() * in.z + translation.y();
    double z =
        rotation[2].x() * in.x + rotation[2].y() * in.y + rotation[2].z() * in.z + translation.z();
...
pcl::PointCloud<PointInT> OcclusionCullingGPU::pointCloudViewportTransform(
    pcl::PointCloud<PointInT> pointCloud, geometry_msgs::Pose cameraPose)
{
    pcl::PointCloud<PointInT> posArray;
    tf::Matrix3x3 rotZ, rotX, rotY, rotE;

    //traslation accroding to the camera position
    tf::Vector3 cameraPoseTrans(-1 * cameraPose.position.x, -1 * cameraPose.position.y,
                                -1 * cameraPose.position.z);
    ...
    //No Rotation
    rotE.setValue(1, 0, 0, 0, 1, 0, 0, 0, 1);

    // rotation of the uav interms of the previous viewport of the pointcloud
    tf::Quaternion qt(cameraPose.orientation.x, cameraPose.orientation.y, cameraPose.orientation.z,
    ...
    rotZ.setValue(std::cos(yaw), -1 * std::sin(yaw), 0, std::sin(yaw), std::cos(yaw), 0, 0, 0, 1);

    // rotation required for frustum culling (not required !)
    double roll = -1 * 90;
    rotX.setValue(1, 0, 0, 0, std::cos(roll), -1 * std::sin(roll), 0, std::sin(roll),
                  std::cos(roll));
    ...
    rotY.setValue(std::cos(pitch), 0, std::sin(pitch), 0, 1, 0, -1 * std::sin(pitch), 0,
                  std::cos(pitch));
...
visualization_msgs::Marker OcclusionCullingGPU::drawLines(std::vector<geometry_msgs::Point> links,
                                                          int id, int c_color[])
{
    visualization_msgs::Marker linksMarkerMsg;
    linksMarkerMsg.header.frame_id =
        "map";  //change to "base_point_cloud" if it is used in component test package
